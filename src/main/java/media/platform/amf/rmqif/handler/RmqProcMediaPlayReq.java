...
//        setData(SessionInfo sessionInfo, int toolId, int mediaType, int[] dstIds, boolean hasContainer, String[] filenames, int defVolume, int lowVolume)
        ...
        if (engineClient == null) {
            ...
            engineClient.pushSentQueue(appId, FilePlayReq.class, filePlayReq.getData());
            if (sessionInfo.getSessionId() != null) {
                AppId.getInstance().push(appId, sessionInfo.getSessionId());
            }

            if (!filePlayReq.send(sessionInfo.getEngineId())) {
                // ERROR
//            EngineClient.getInstance().removeSentQueue(appId);
            }

            logger.info("[{}] -> (Engine-{}) FilePlayReq. toolId [{}] filename [{}]", sessionInfo.getSessionId(),
                    sessionInfo.getEngineId(), sessionInfo.getEngineToolId(), filenames);
        }
