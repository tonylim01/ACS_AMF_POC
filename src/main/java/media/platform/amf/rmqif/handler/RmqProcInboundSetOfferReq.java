...
public class RmqProcInboundSetOfferReq extends RmqIncomingMessageHandler {
    ...
    public boolean handle(RmqMessage msg) {
        ...
        boolean isResoureFull = false;

        if ((sdpInfo != null) && (sdpInfo.getCodecStr() != null)) {
            if (sdpInfo.getCodecStr().equals("AMR-WB") || sdpInfo.getCodecStr().equals("AMR-NB")) {
                if (EngineManager.getInstance().getAmrIdle() <= 0) {
                    isResoureFull = true;
                }
            } else if (sdpInfo.getCodecStr().equals("EVS")) {
                if (EngineManager.getInstance().getEvsIdle() <= 0) {
                    isResoureFull = true;
                }
            }
        }
        ...
        if (isResoureFull) {
            UaOamManager.sendTrace((req.getOutbound() == 2) ? UaTraceMsg.DIR_OUT : UaTraceMsg.DIR_IN,
                    msg.getHeader().getType(),
                    msg.getHeader().getMsgFrom(), null,
                    req.getFromNo(), req.getToNo(),
                    "");

            sendResponse(msg.getSessionId(), msg.getHeader().getTransactionId(), msg.getHeader().getMsgFrom(),
                    RmqMessageType.RMQ_MSG_COMMON_REASON_CODE_RESOURCE_FULL,
                    "Codec resource full");
            return false;
        }
        ...
        sessionInfo.setEngineId((roomInfo != null) ? roomInfo.getEngineId() : 0);
        ...
        do {
            try {
                ...
                sessionInfo.rtpChannel = AppInstance.getInstance().getNettyRTPServer().addBindPort("0.0.0.0", localPort);
