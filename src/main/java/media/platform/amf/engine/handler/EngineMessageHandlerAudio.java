...
public class EngineMessageHandlerAudio extends BaseEngineMessageHandler {

    private static final Logger logger = LoggerFactory.getLogger(EngineMessageHandlerAudio.class);

    private int index;

    public EngineMessageHandlerAudio(int index) {
        this.index = index;
    }
    ...
    public void handle(EngineReportMessage msg) {
        ...
        if (compareString(msg.getHeader().getCmd(), EngineMessageType.MSG_CMD_BRANCH)) {
            procAudioBranchRpt(sessionId, msg);
        }
        else {
            logger.warn("Unsupported cmd [{}]", msg.getHeader().getCmd());
        }
    ...
    private void procAudioCreateRes(String sessionId, EngineResponseMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
...
//                    EngineServiceManager.getInstance().popAndSendMessage();
            ...
            new EventHandler().onApplicationEvent(Level.MIN,
                    String.format("AMF engine audio error [%s]", msg.getHeader().getReason()));
        }

    }

    private void procAudioDeleteRes(String sessionId, EngineResponseMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
    ...
    private void procAudioBranchRes(String sessionId, EngineResponseMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
    ...
    private void procAudioBranchRpt(String sessionId, EngineReportMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
        ...
        if (sessionInfo.getConferenceId() != null) {
            ...
            if (roomInfo != null && roomInfo.getAwfQueueName() != null) {
                ...
                logger.warn("[{}] Resend wakeup. isCaller [{}] caller [{}] callee [{}] wakeupStatus [{}]", sessionInfo.getSessionId(),
                ...
                if (otherSessionInfo != null) {
                    ...
                    logger.warn("[{}] Resend wakeup. isCaller [{}] caller [{}] callee [{}] wakeupStatus [{}]", otherSessionInfo.getSessionId(),
    ...
    private void sendWakeupStartReqToEngine(SessionInfo sessionInfo, int toolId) {
        ...
        EngineClient engineClient = EngineClient.getInstance(index);
        if (engineClient == null) {
            logger.error("[{}] Engine not found", sessionInfo.getSessionId());
            return;
        }

        engineClient.pushSentQueue(appId, WakeupStartReq.class, wakeupStartReq.getData());
        if (sessionInfo.getSessionId() != null) {
            AppId.getInstance().push(appId, sessionInfo.getSessionId());
        }

        if (!wakeupStartReq.send(index, false)) {
            // ERROR
//            AppId.getInstance().remove(appId);
//            EngineClient.getInstance().removeSentQueue(appId);
        }

        logger.info("[{}] -> (Engine-{}) WakeupStartReq. toolId [{}]", sessionInfo.getSessionId(), index, toolId);
