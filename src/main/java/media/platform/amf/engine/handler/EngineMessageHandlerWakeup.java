...
public class EngineMessageHandlerWakeup extends BaseEngineMessageHandler {

    private static final Logger logger = LoggerFactory.getLogger(EngineMessageHandlerWakeup.class);

    private int index;

    public EngineMessageHandlerWakeup(int index) {
        this.index = index;
    }
    ...
    public void handle(EngineReportMessage msg) {
        ...
        if (compareString(msg.getHeader().getCmd(), EngineMessageType.MSG_CMD_START)) {
            procWakeupStartRpt(sessionId, msg);
        }
        else {
            logger.warn("Unsupported cmd [{}]", msg.getHeader().getCmd());
        }
    ...
    private void procWakeupStartRes(String sessionId, EngineResponseMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
            ...
            if (sessionInfo.getConferenceId() != null) {
                ...
                if (roomInfo != null) {
                    ...
                    if ((wakeupStatus & 0x5) == 0) {
                        ...
                        RmqProcWakeupStatusRes res = new RmqProcWakeupStatusRes((roomInfo.getAwfCallId() != null) ? roomInfo.getAwfCallId() : sessionId,
                                roomInfo.getLastTransactionId());
    ...
    private void procWakeupStopRes(String sessionId, EngineResponseMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
    ...
    private void procWakeupStartRpt(String sessionId, EngineReportMessage msg) {
        if (msg == null || msg.getHeader() == null) {
            logger.warn("Null response message");
            return;
        }
        ...
        if (compareString(msg.getHeader().getEvent(), EngineMessageType.MSG_EVENT_DETECTED)) {
            ...
            if (sessionInfo.getConferenceId() != null) {
                ...
                if (roomInfo != null && roomInfo.getAwfQueueName() != null) {
                    ...
                    roomInfo.setWakeupStatus(sessionInfo.isCaller() ? true : false, RoomInfo.WAKEUP_STATUS_NONE);

                    String appId = AppId.newId();

                    RmqProcAiServiceReq req = new RmqProcAiServiceReq((roomInfo.getAwfCallId() != null) ? roomInfo.getAwfCallId() : sessionInfo.getSessionId(), appId);
                    req.send(roomInfo.getAwfQueueName(), sessionInfo.isCaller() ? 1 : 2);

                    StatManager.getInstance().incCount(StatManager.SVC_AI_REQ);

                    TransactionManager.getInstance().put(appId, sessionInfo.getSessionId());
                }
            }
        }
        else {
