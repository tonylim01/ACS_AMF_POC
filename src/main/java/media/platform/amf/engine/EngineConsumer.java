...
import media.platform.amf.engine.handler.*;
...
public class EngineConsumer implements Runnable {
    ...
    public EngineConsumer(int index, BlockingQueue<byte[]> queue) {
        this.index = index;
        this.queue = queue;
    ...
    @Override
    public void run() {
        logger.debug("EngineConsumer [{}] startScheduler", index);
...
//                handleSysResponse(msg);
                EngineMessageHandlerSys sys = new EngineMessageHandlerSys(index);
                sys.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_AUDIO)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerAudio mixer = new EngineMessageHandlerAudio(index);
                mixer.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_MIXER)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerMixer mixer = new EngineMessageHandlerMixer(index);
                mixer.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_FILE)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerFile wakeup = new EngineMessageHandlerFile(index);
                wakeup.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_WAKEUP)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerWakeup wakeup = new EngineMessageHandlerWakeup(index);
                wakeup.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_TOOL)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerTool tool = new EngineMessageHandlerTool(index);
                tool.handle(msg);
            }
            else {
                logger.debug("<- Engine: json {}", json);
            ...
            if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_AUDIO)) {
                logger.debug("<- Engine: json {}", json);
                EngineMessageHandlerAudio audio = new EngineMessageHandlerAudio(index);
                audio.handle(msg);
            }
            else if (compareString(msg.getHeader().getType(), EngineMessageType.HDR_TYPE_FILE)) {
                logger.debug("<- Engine: json {}", json);
