...
public class EngineClient {
    ...
    private volatile static Map<Integer, EngineClient> engineClients = null;
    ...
    public static void init(int clientCount) {
        logger.info("Engine client count [{}]", clientCount);
        if (clientCount > 0) {
            engineClients = new HashMap<>(clientCount);
        }
        else {
            logger.error("Invalid client count [{}]", clientCount);
        }
    }

    public static EngineClient getInstance(int index) {

        if (engineClients == null) {
            return null;
        }

        EngineClient engineClient = null;

        if (engineClients.get(index) == null) {
            UserConfig config = AppInstance.getInstance().getUserConfig();
            if (config.getEngineIp(index) != null && config.getEngineRemotePort(index) > 0) {
                engineClient = new EngineClient(index, config.getEngineIp(index), config.getEngineRemotePort(index));
                engineClients.put(index, engineClient);
            }
            else {
            ...
            engineClient = engineClients.get(index);
        }
    ...
    public EngineClient(int index, String ip, int port) {
        this.engineId = index;
        this.ip = ip;
        this.port = port;
    ...
    public boolean sendMessage(String msg, boolean printLog) {
        ...
        if (printLog) {
            logger.debug("-> (Engine-{}) [{}] result [{}]", engineId, msg, result);
        }
    ...
    class EngineClientRunnable implements Runnable {
        ...
        public void run() {
            ...
            logger.info("EngineClient startScheduler. id [{}] ip [{}] remote [{}]",
                    engineId, ip, port);
            ...
            while (!isQuit) {
                ...
                if (!isConnected()) {
                    EngineProcSysConnectReq sysConnectReq = new EngineProcSysConnectReq(appId, engineId);

                    if (sysConnectReq.send(engineId)) {
                    ...
                    logger.warn("No heartbeat response from engine. count[{}]", retryCount);
                ...
                if (isHeartbeat) {
                    ...
                    if (sysHeartbeatReq.send(engineId)) {
