...
public class RoomManager {
    ...
    public int addSession(String roomId, String sessionId) {
        ...
        synchronized (roomInfos) {
            ...
            if (roomInfos.containsKey(roomId)) {
                ...
                int engineId = EngineManager.getInstance().getIdleEngineIndex();
                roomInfo.setEngineId((engineId >= 0) ? engineId : 0);

                roomInfos.put(roomId, roomInfo);

                logger.debug("[{}] New Room. engineId [{}] Count [{}]", roomId, engineId, roomInfos.size());
    ...
    public int getEngineId(SessionInfo sessionInfo) {
        if (sessionInfo == null) {
            return -1;
        }
    ...
    public int getEngineId(String sessionId) {
        SessionInfo sessionInfo = SessionManager.getInstance().getSession(sessionId);
        return (sessionInfo != null) ? getEngineId(sessionInfo) : -1;
    }

    public SessionInfo getProperSessionInfo(SessionInfo refSessionInfo, boolean isCaller) {
        if (refSessionInfo == null) {
            return null;
        }
