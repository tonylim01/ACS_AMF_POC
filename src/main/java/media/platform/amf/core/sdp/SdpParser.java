...
public class SdpParser {
        ...
        try {
            ...
            if (sdpInfo == null) {
                new EventHandler().onApplicationEvent(Level.MIN,
                        String.format("SDP paring error [%s]", sdp));
                return null;
            }
            ...
            if (sdpInfo.getAttributes() != null) {
                ...
                if (mediaPriorities != null && mediaPriorities.size() > 0) {
                    ...
                    if (sdpInfo.getPayloadId() < 0) {
                        new EventHandler().onApplicationEvent(Level.MIN,
                                String.format("SDP nego error [No configured media found]"));

                        sdpInfo.setPayload2833(0);
                    }
                }
                else {
    ...
    private static boolean matchSdp(SdpInfo sdpInfo, String priorityCodec) {
        ...
        if (attr != null) {
            if (sdpInfo.getPayloadId() < 0) {
                ...
                logger.debug("Match found. PayloadId [{}]", attr.getPayloadId());
...
//            sdpInfo.setPriority(i);
                ...
                sdpInfo.addMediaFirst(priorityCodec);
            }
            else {
                sdpInfo.addMedia(priorityCodec);
            }
