...
public class JitterSender {
    ...
    private class LoopProc implements Runnable {
        ...
        public void run() {
            ...
            logger.debug("[{}] LoopProc startScheduler", sessionId);
            int tick = 0;
...
//                    logger.debug("packet found. client ({}) data ({}) relay {}", udpPacket != null, udpPacket.getData() != null, relay);
                    ...
                    if ((udpPacket.getSeqNo() > 0) && (lastSeq > 0) && (udpPacket.getSeqNo() - lastSeq > 1)) {
                        int statType = 0;
                        switch (vocoder) {
                            case Vocoder.VOCODER_AMR_WB:
                            case Vocoder.VOCODER_AMR_NB:
                                statType = isCaller ? StatManager.RTP_AMR_IN_LOSS : StatManager.RTP_AMR_OUT_LOSS;
                                break;
                            case Vocoder.VOCODER_EVS:
                                statType = isCaller ? StatManager.RTP_EVS_IN_LOSS : StatManager.RTP_EVS_OUT_LOSS;
                                break;
                            default:
                                statType = isCaller ? StatManager.RTP_ETC_IN_LOSS : StatManager.RTP_ETC_OUT_LOSS;
                                break;
                        }

                        StatManager.getInstance().incCount(statType, udpPacket.getSeqNo() - lastSeq);
                    }

                    lastSeq = udpPacket.getSeqNo();
...
//                    logger.debug("-> tick {} remaining {} delay {}", tick, buffer.size(), delay);
            ...
            logger.debug("[{}] LoopProc end", sessionId);
        }
    ...
    private void relayPacket(UdpPacket udpPacket) {
        ...
        if ((udpClient != null) && (rtpPacket != null) &&
                (AppInstance.getInstance().getConfig().isTest() == false)) {
            if (isSkip == false) {
